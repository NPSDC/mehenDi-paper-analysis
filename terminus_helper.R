...
computeTermAggNodes <- function(groups, nodeID, se_counts, group_inds = NULL) {
...
prepTerm <- function(y, inds, groups)
{
    ...
    for(n in names(asList)) {
        if(n!="length")
            asList[[n]] <- computeTermAggNodes(groups, inds, assays(y)[[n]], NULL)
        else {
            abundLength <- assays(y)[["abundance"]] * assays(y)[["length"]]
            weightedLength <- computeTermAggNodes(groups, inds, abundLength, NULL)
